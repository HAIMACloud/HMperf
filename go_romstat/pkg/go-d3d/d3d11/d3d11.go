...
func _D3D11CreateDevice(ppDevice **ID3D11Device, ppDeviceContext **ID3D11DeviceContext) error {
	...
	ret, _, _ := syscall.SyscallN(
		//Use Hardware driver for default
		procD3D11CreateDevice.Addr(),
		uintptr(0),                          //no use adapter
		uintptr(1),                          //driverType: Hardware
		uintptr(0),                          // software
		uintptr(flags),                      // flags
	...
	if ret != 0 {
		//Not support Hardware Type, use first adapter， and don't use fflags
		ret, _, _ := syscall.SyscallN(
			procD3D11CreateDevice.Addr(),
			...
			uintptr(0),                        // software
			uintptr(flags),                    // flags
			uintptr(0),                        // supported feature levels (BUGFIX:不指定版本，如未安装对应版本会导致报错)
			uintptr(0),                        // number of levels
			uintptr(D3D11_SDK_VERSION),
			uintptr(unsafe.Pointer(ppDevice)),        // *D3D11Device
			uintptr(unsafe.Pointer(&featureLevel)),   // feature level
			uintptr(unsafe.Pointer(ppDeviceContext)), // *D3D11DeviceContext
